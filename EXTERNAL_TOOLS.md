...
`datetime`, `bash`, `python3`, `rag`, `tool_lookup`). Those are the tools we
...
| --- | --- | --- |
| `name` | yes | Identifier the model uses. `^[a-zA-Z][a-zA-Z0-9_]{0,63}$`. Cannot collide with built-ins (`bash`, `python3`, `web`, `fs`, `rag`, `datetime`, `tool_lookup`) or with tools declared in earlier-sorted files. |
