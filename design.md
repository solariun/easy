...
| **libcurl** (optional) | the unified `web` tool (action=`search` / `fetch`; `engine="auto"` default cascades google → brave → bing → ddg, plus explicit `engine="google"` / `"brave"` / `"bing"` / `"ddg"` overrides) | system package (`libcurl-dev`) | `libeasyai.so` `PRIVATE` when `EASYAI_WITH_CURL=ON`      |
