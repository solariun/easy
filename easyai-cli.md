...
| `--tools-mode MODE` | How `fs` / `web` / `memory` are exposed to the model. **MODE** is one of `unified` (default; single dispatcher per family with `action=`), `split` (one focused tool per action — `fs_read`, `fs_edit`, …, `memory_save`, …, `web_search`, `web_fetch`), or `both` (register both surfaces side-by-side). Same handlers under the hood; the trade is system-prompt size vs. one-verb-per-tool clarity for smaller / quantised callers. INI: `[cli] tools_mode = unified\|split\|both`. |
...
| `tools_mode`    | `unified` | `--tools-mode unified\|split\|both` | How `fs` / `web` / `memory` are exposed: `unified` (single dispatcher per family with `action=`), `split` (one focused tool per action — `fs_read`, `fs_edit`, etc.), or `both`. Smaller / quantised tool-callers consistently work better with `split`; large models handle either. Same handlers — only the registration shape changes. |
