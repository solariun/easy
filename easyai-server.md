...
| `allow_python` | bool | `--allow-python` | `off` | Register the `python3` tool — runs snippets via `python3 -I -S -E -c <code>`. Isolated stdlib-only interpreter: no PYTHON* env, no site-packages, no cwd on `sys.path`; `import requests` etc. fail with ModuleNotFoundError, by design. **Not** a hardened sandbox: `import os` / `import socket` / `import subprocess` all work. Same fs-registration caveat as `allow_bash` above. |
...
| `--allow-python` (`[SERVER] allow_python`) | `python3` (run snippets via `python3 -I -S -E -c <code>`). Isolated stdlib-only interpreter — no PYTHON* env, no site-packages, no cwd on `sys.path`. cwd = sandbox if given, else the binary's CWD. NOT a hardened sandbox — `import os` / `import socket` / `import subprocess` all work. Bumps `max_tool_hops` to 99999, same as `--allow-bash`. Does NOT auto-register `fs`. |
