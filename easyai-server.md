...
| `load_tools` | bool | `--no-tools` (negative) | `on` | Master switch for the built-in toolbelt. Set `off` (or pass `--no-tools`) to register zero default tools. `allow_fs` / `allow_bash` / `use_google` further opt in. |
...
| `--no-tools` (`[SERVER] load_tools = off`) | Skip the built-in toolbelt entirely. Useful when you want ONLY external-tools or ONLY RAG. |
