        ...
        if (args.webui_mode != "minimal") {
            ...
            inj <<
              "<script>(()=>{"
                ...
                // --- overview stats bar (always visible, above textarea) ---
                // Fixed-position Shadow DOM panel showing context utilisation
                // and the last response's tokens / time / t/s.  Updated by
                // monitorSSE every time it sees `timings` in the stream.
                "const OV_ID='__easyaiOverviewHost';"
                "let ovRoot=null;"
                "let lastTimings=null;"
                ...
                "const ensureOverview=()=>{"
                  "let h=document.getElementById(OV_ID);"
                  "if(h&&ovRoot)return ovRoot;"
                  "if(!document.documentElement)return null;"
                  "h=document.createElement('div');"
                  "h.id=OV_ID;"
                  "h.style.cssText="
                    "'all:initial;position:fixed;bottom:5.6rem;'+"
                  ...
                  "root.innerHTML="
                    "'<style>"
                      ":host,*{box-sizing:border-box}"
                      ".bar{display:inline-flex;align-items:center;gap:.55rem;"
                ...
                "function renderOverview(){"
                  "const r=ensureOverview();if(!r)return;"
                ...
                "window.__easyaiPushTimings=(t)=>{"
                  ...
                  "renderOverview();"
                "};"
                "if(document.documentElement)ensureOverview();"
                "document.addEventListener('DOMContentLoaded',()=>{"
                  "ensureOverview();renderOverview();"
                "});"
                "setInterval(()=>{"
                  "if(!document.getElementById(OV_ID)){ovRoot=null;ensureOverview();renderOverview();}"
                "},1500);"
              "})();</script>";
