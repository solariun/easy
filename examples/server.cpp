...
static std::string build_builtin_system_prompt(const ServerArgs & args) {
    ...
    s +=
        ...
        "listing the URLs you ACTUALLY read — one per line, in citation\n"
        "order. Don't invent URLs. Don't include results you only saw\n"
        "in a snippet but never fetched. If you answered from training\n"
        "or memory alone (no external lookup this turn), no sources\n"
        "block.\n"
        "\n"
