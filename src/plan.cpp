...
Tool Plan::tool() {
    ...
    return Tool::make(
        ...
        "assistant's reply). USE whenever a task has 3+ distinct steps "
        "you can name in advance, or when the user asks you to plan / "
        "track / break down work. SKIP for one-shot answers, single tool "
        "calls, or pure information lookups.\n"
        "\n"
        "CRITICAL — never violate:\n"
        "  • Each step's 'text' is HARD-CAPPED at 80 chars AND must be "
        "ONE short imperative phrase (e.g. \"write Makefile\"). NEVER "
        "cram a numbered/comma list or multi-step plan into one 'text' — "
        "the server REJECTS it. Track several steps as separate entries: "
        "items=[{text:\"step 1\"},{text:\"step 2\"},…].\n"
        "  • 'items' MUST be a real JSON array, NEVER a quoted string "
        "containing JSON. Max 20 entries.\n"
        "  • Each step has an INTEGER id assigned by 'add' (\"1\", "
        "\"2\", …); ids are stable. NEVER invent an id — if unsure, "
        "action=\"list\" first (read-only) and update by the ids you "
        "saw.\n"
        "  • Mark STARTED: update id=\"N\" status=\"working\" — AT MOST "
        "ONE step 'working' at a time; finish the current one before "
        "starting the next. Mark FINISHED: update id=\"N\" "
        "status=\"done\" — NEVER call 'add' again for the same step "
        "(that duplicates it).\n"
        "  • Prefer ONE batched call (items=[…]) over N separate "
        "tool_calls when mutating several steps.\n"
        "\n"
        "Pick EXACTLY ONE 'action':\n"
        "  action=\"add\"  — append new pending step(s).\n"
        "    Single: {action:\"add\", text:\"do X\"}\n"
        "    Batch:  {action:\"add\", items:[{text:\"a\"},{text:\"b\"}]}\n"
        "    Optional per item: status (default \"pending\"; \"working\" "
        "starts it in the same call). NEVER use 'add' to mark a step "
        "done — use 'update'.\n"
        ...
        "  action=\"delete\"  — remove step(s). {action:\"delete\", "
        "id:\"2\"} | id:\"all\" wipes all | items:[{id:\"2\"},"
        "{id:\"4\"}].\n"
        "  action=\"list\"  — return the current plan. READ-ONLY. "
        "{action:\"list\"}\n"
        "\n"
        "Statuses: pending (default) | working (exactly one at a time) | "
        "done | error | deleted (prefer action=\"delete\").",
        kSchema,
